...
public partial class MasterProductEdit : System.Web.UI.Page
{
    ...
    protected void cboSupplier_OnItemsRequested(object sender, RadComboBoxItemsRequestedEventArgs e)
    {
        try
        {
            ...
            connMenu.Open();
            string sql = "";
            sql = "select SupplierName,SupplierID from SupplierDetails where deleted=0 and SupplierName LIKE @Text + '%' group by SupplierName,SupplierID order by SupplierID desc";
            SqlDataAdapter sqlDataAdapter = new SqlDataAdapter(sql, connMenu);
            sqlDataAdapter.SelectCommand.Parameters.AddWithValue("@Text", e.Text);
    ...
    protected void cboBrand_OnItemsRequested(object sender, RadComboBoxItemsRequestedEventArgs e)
    {
        ...
        {
            InsertLogAuditTrail(Session["sesUserID"].ToString(), "ProductItemMaster", "cboBrand_OnItemsRequested", ex.ToString(), "Audit");
        }
    }
    ...
    protected void txtCatalogName_OnItemsRequested(object sender, RadComboBoxItemsRequestedEventArgs e)
    {
        try
        {
            ...
            connMenu.Open();
            string sql = "";
            sql = "select Catalogid,CatalogName FROM CatalogMaster WHERE Deleted=0 and CatalogName LIKE @Text + '%' order by Catalogid desc";
            SqlDataAdapter sqlDataAdapter = new SqlDataAdapter(sql, connMenu);
            sqlDataAdapter.SelectCommand.Parameters.AddWithValue("@Text", e.Text);
            DataTable g_datatable = new DataTable();
            sqlDataAdapter.Fill(g_datatable);
    ...
    protected void btnAddProduct_OnClick(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            if (string.IsNullOrEmpty(txtSGCutPrice.Text.ToString().Trim()))
            {
                txtSGCutPrice.Text = "0";
            }
            if (string.IsNullOrEmpty(txtSGPrice.Text.ToString().Trim()))
            {
                txtSGPrice.Text = "0";
            }
    ...
    protected void btnEdit_Onclick(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            if (reader.Read())
            {
                ...
                if (Convert.ToBoolean(reader["Singapore"].ToString()) == true)
                {
                ...
                txtSGCutPrice.Text = reader["SGCutPrice"].ToString();
                txtSGPrice.Text = reader["SGPrice"].ToString();
                Img.ImageUrl = lnkDownload1.Text;
                if (string.IsNullOrEmpty(lnkDownload1.Text.ToString().Trim()))
                {
                    flProductPhoto.Visible = true;
