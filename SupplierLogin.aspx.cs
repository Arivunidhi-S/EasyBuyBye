...
public partial class SuplierLogin : System.Web.UI.Page
{
    ...
    protected void btnSignin_OnClick(object sender, EventArgs e)
    {
        ...
        conn.Open();
        SqlDataReader reader1 = BusinessTier.VaildateUserLogin(conn, txtUserID.Text.ToString(), txtPassword.Text.ToString(), "", "Seller");
        
         if (reader1.Read())
        {
            Session["sesUserID"] = (reader1["Name"].ToString());
            Session["CustomerID"] = reader1["SellerID"].ToString(); 
        ...
        {
            InsertLogAuditTrail(Session["sesUserID"].ToString(), "SuplierLogin", "btnSignin_OnClick", ex.ToString(), "Audit");
